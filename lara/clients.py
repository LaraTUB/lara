...
class GithubClient(object):
    ...
    @staticmethod
    def list_org_issues():
        return [issue._rawData for issue in organization.get_issues()]

    @staticmethod
    def list_repo_issues():
        return [issue._rawData for issue in repo.get_issues()]

    @staticmethod
    def create_repo_issue(title, body):
        return repo.create_issue(title, body)._rawData

    @staticmethod
    def update_repo_issue(issue_id, title, body):
        issue = repo.get_issue(issue_id)
        issue.edit(title, body)
        return repo.get_issue(issue_id)._rawData
